...
namespace BankingApp.AccountManagement.Infrastructure.Repositories
{
    public class AccountRepository<T> : IAccountRepository<T> where T : EntityBase, IAccountNonGenericRepo
    {
        ...
        public async Task<List<Account>> GetAccountsWithDetailsAsync(Expression<Func<Account, bool>> predicate)
        {
            var accounts = await _context.Set<Account>()
                .Include(a => a.CreditsCollection)
                .Include(a => a.DebitsCollection)
                .Where(predicate)
                .ToListAsync();
