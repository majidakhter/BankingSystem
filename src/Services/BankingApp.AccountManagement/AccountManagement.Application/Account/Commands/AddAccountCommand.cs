...
{
    public sealed record AddAccountCommand(Guid customerId, Guid keycloakUserId, int accountTypeId) : CreateCommand;
    ...
    public sealed class AddAccountCommandHandler : CreateCommandHandler<AddAccountCommand>
    {
        ...
        protected override async Task<Guid> HandleAsync(AddAccountCommand request)
        {
            ...
            var created = Account.Create(request.customerId,request.keycloakUserId, request.accountTypeId);
